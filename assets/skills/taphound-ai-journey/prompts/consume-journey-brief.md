...
4. Require these Markdown sections exactly once (every Brief, regardless of
   `schemaVersion`):
   - `# Goal`
   - `## Preconditions`
   - `## Expected Journey`
   ...
   - `## Constraints`
   - `## Evidence References`
5. When `schemaVersion: 2`, additionally require each of these exactly once:
   - `## State Transition Map`
   - `## Capability Notes`
   A v1 Brief omits both and remains valid; do not require them for v1.
...
- For v2 `## State Transition Map` edges marked `needs-observation`, the
...
- For v2 `## Capability Notes`, treat the entries as hints only. Core behavior
