...
- 审计日期：2026-07-19
- 依据：[已批准设计](../plans/2026-07-19-apr-v0.2-design.md) 与[实施计划](../plans/2026-07-19-apr-v0.2-implementation.md)
...
|---|---|---|---|---|
| 1 | APR 交互式 Recorder 生成包含 Action、Locator 和 Activity Checkpoint 的 Journey | [`recorder-service.ts`](../../src/application/recorder/recorder-service.ts)、[`locator-selector.ts`](../../src/application/recorder/locator-selector.ts)、[`inquirer-recorder-prompt.ts`](../../src/adapters/prompt/inquirer-recorder-prompt.ts) | [`recorder-service.test.ts`](../../test/application/recorder/recorder-service.test.ts) 覆盖构建/启动、操作执行、前后 Activity、只保存成功步骤、Finish 原子写入、Cancel；[`locator-selector.test.ts`](../../test/application/recorder/locator-selector.test.ts) 覆盖 Locator 唯一性和优先级 | 已证明 |
...
| 6 | 按步骤切片并匹配 Logcat | [`logcat-collector.ts`](../../src/application/collector/logcat-collector.ts) 记录单调接收时间并切片；[`step-runner.ts`](../../src/application/runtime/step-runner.ts) 写步骤日志；Expect evaluator 匹配 tag/level/pattern | [`logcat-collector.test.ts`](../../test/application/collector/logcat-collector.test.ts) 覆盖含边界的 `[T0,T1]` 切片和 PID 范围；[`step-runner.test.ts`](../../test/application/runtime/step-runner.test.ts) 与 Expect 测试覆盖步骤级消费 | 已证明 |
...
| 约束 | 证据 | 结论 |
|---|---|---|
| APR Journey 完全自研，不调用或兼容官方 Journey | 严格 [`JourneySchema`](../../src/domain/journey.ts) 及其拒绝额外/自然语言格式测试；文档在 [`journey-schema.md`](../journey-schema.md) 明确区分 | 已证明 |
...
| 外部命令不经 Shell、支持超时与取消 | [`node-process-runner.ts`](../../src/adapters/process/node-process-runner.ts) 直接 `spawn` executable/args；[`node-process-runner.test.ts`](../../test/adapters/process/node-process-runner.test.ts) 覆盖参数保真、超时、AbortSignal 和流式输出 | 已证明 |
...
```text
node dist/cli/main.js doctor --project examples/apr-demo --json
```
...
- 未设置 `APR_ACCEPTANCE_DEVICE=1`，也未运行 `npm run acceptance:device`；没有真实设备报告路径可附。
